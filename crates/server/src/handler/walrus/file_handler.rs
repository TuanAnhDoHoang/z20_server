...
use crate::dtos::file_dto::{BlobInfomation, RequestUpload};
pub async fn handle_upload_chunk(data: RequestUpload) -> Result<(bool, BlobInfomation)> {
    let upload_id = data.uploadId();
    let identifier = data.identifier();
    let prototype_code = data.prototypeCode();

    let project_dir = format!("/tmp/uploads/{upload_id}");
    let output_path = format!("{project_dir}/{identifier}");
    ...
    // Write prototype_code to file
    output_file.write_all(prototype_code.as_bytes())?;
    
    let mut result = BlobInfomation::default();
    result.set_identifier(identifier.to_owned());
    result.set_blob_name(identifier.to_owned());
